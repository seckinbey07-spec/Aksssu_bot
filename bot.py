...
def ilan_search():
    ...
    for ad in ads:
        ...
        try:
            page = requests.get(full_url, timeout=20, headers={"User-Agent": "Mozilla/5.0"})
            soup = BeautifulSoup(page.text, "html.parser")
            page_text = soup.get_text(" ", strip=True).lower()

            # Sayfa içinde Antalya geçiyorsa kabul
            if "antalya" in page_text:
                results.append({
                    "id": str(ad.get("id")),
                    "title": ad.get("title"),
                    "url": full_url
...
def main():
    ...
    for it in new_items:
        send_telegram(f"🆕 Antalya kiralama:\n{it['title']}\n{it['url']}")
        seen.add(it["id"])
        time.sleep(1)
